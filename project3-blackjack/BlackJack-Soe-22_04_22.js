 ...
 * @returns {array} An array of playerRecords that are eliminated
 */
var getEliminatedPlayers = function () {
  ...
  return playerRecords.filter(criterionToEliminate);
};
...
var getRemainingPlayers = function () {
  ...
  return playerRecords.filter(criterionToRemain);
};
...
var main = function (input) {
  ...
  if (mode == stateOne) {
    ...
    if (isNaN(input) == true || !Number(input) > 0) {
      return "Please enter a number of player larger than 0";
    }
